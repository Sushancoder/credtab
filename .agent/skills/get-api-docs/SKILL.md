...
name: get-api-docs
description: >
  Use this skill when you need documentation for a third-party library, SDK, or API before writing code that uses it — for example, "use the OpenAI API", "call theStripe API", "use the Anthropic SDK", "use the Gemini SDK", "query Pinecone", or any time the user asks you to write code against an external service and you need current API reference. 
...
```bash
chub get <id> --lang ts    # or --lang py, --lang js
```
...
| Fetch TypeScript docs | `chub get stripe/api --lang ts` |
...
- Prefer `--lang ts` for this project (Next.js + TypeScript)
